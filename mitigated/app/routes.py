"""Main routes — HARDENED build.

Three textbook fixes are wired in:

  ✅ FIX #1  — SQL Injection             → parameterized queries (`?`) in /search
  ✅ FIX #2  — Reflected XSS             → Jinja2 auto-escape (no |safe) + CSP header
  ✅ FIX #3a — CSRF                      → token enforced globally in __init__.py
  ✅ FIX #3b — Mass-assignment           → /admin/users/set-role validates
              target_role against a whitelist of {'ta','student'}.
"""
...
ALLOWED_ROLES_FOR_SET_ROLE = {"student", "ta"}   # NEVER includes "admin".
...
def search():
    ...
    if q:
        # ✅ FIX #1 — parameterized query. User input bound via `?` placeholders.
        sql_used = (
        ...
        like = f"%{q}%"
        try:
            rows = get_db().execute(sql_used, (like, like)).fetchall()
...
def withdraw_student(course_id):
    ...
    db = get_db()
    # Policy: TAs cannot withdraw a graded student. Admin can (final authority).
    row = db.execute(
...
# ─────────────────────────────────────────────────────────────────────────
# ✅ FIX #3 — CSRF + Mass-Assignment   /admin/users/set-role
# ─────────────────────────────────────────────────────────────────────────
...
def admin_user_set_role():
    ...
    if target_id <= 0 or target_role not in ALLOWED_ROLES_FOR_SET_ROLE:
        flash("Invalid request — role must be 'student' or 'ta'.", "error")
        return redirect(url_for("main.admin_users"))
    ...
    if not target:
        ...
        return redirect(url_for("main.admin_users"))
    if target["role"] == "admin":
        flash("Admin role can't be changed through this endpoint.", "error")
        return redirect(url_for("main.admin_users"))
